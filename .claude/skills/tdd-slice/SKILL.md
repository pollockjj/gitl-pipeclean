...
**Local test runs are development feedback. CI test runs are official evidence.** Run tests freely during development to verify your code. When you are satisfied the code is correct, push to trigger CI. Only CI-committed artifacts are evaluated by the QA gate.

#### Workflow

1. **Develop and test locally** — run tests as needed during implementation to verify correctness
2. **Create a branch** named `issue{ISSUE_NUMBER}/slice{N}` from main
3. **Commit your code changes** to that branch
4. **Push the branch** — this triggers the `.github/workflows/slice-evidence.yml` workflow
5. **Wait for CI to complete** — poll with `gh run list --branch issue{ISSUE_NUMBER}/slice{N}` until status is `completed`
6. **Verify CI success** — `gh run view {RUN_ID} --json conclusion --jq '.conclusion'` must be `success`
...
**You do NOT commit evidence artifacts.** CI commits evidence. Your local test output is for your development loop. The QA gate evaluates only CI-committed artifacts.
